...
public class ImageUtils {
	...
	public static BufferedImage toBufferImage(String imgUrl) {
	...
	public static BufferedImage catImage(BufferedImage image, int x, int y,
			int wight, int hight) {
		...
		if (originalWidth < wight) {
			wight = originalWidth;
		}
		if (originalHeight < hight) {
			hight = originalHeight;
		}

		int[] simgRgb = new int[wight * hight];
		image.getRGB(x, y, wight, hight, simgRgb, 0, wight);
		BufferedImage img = new BufferedImage(wight, hight, image.getType());
		img.setRGB(0, 0, wight, hight, simgRgb, 0, wight);
		return img;
	...
	public static BufferedImage scaleImage(BufferedImage image,
		...
		if (scaleFactor < 1
			...
			// Math.round(originalWidth * scaleFactor), (int)
	...
	public static void invertImage2(BufferedImage image) {
		WritableRaster inRaster = image.getRaster();
		int[] p = new int[3];
		// float v = 0;
		for (int x = 0; x < inRaster.getWidth(); x++) {
	 ...
	 */
	public static BufferedImage trimWhiteSpace(BufferedImage img,
			int whiteThreshold, int startTop, int startRight, int startBottom,
			int startLeft) {
		return trimWhiteSpace(img, img, whiteThreshold, startTop, startRight,
				startBottom, startLeft);
	}

	public static BufferedImage trimWhiteSpace(BufferedImage src,
			BufferedImage tgt, int whiteThreshold, int startTop,
			int startRight, int startBottom, int startLeft) {
		...
		result.getGraphics().drawImage(tgt, 0, 0, result.getWidth(),
				result.getHeight(), trimLeft, trimTop,
				src.getWidth() - trimRight, src.getHeight() - trimBottom, null);
		return result;
	}
	...
	public static BufferedImage denoiseImage(BufferedImage img) {
		...
		for (int x = 0; x < raster.getWidth(); x++) {
			...
			for (int y = 0; y < raster.getHeight(); y++) {
				...
				if (pCol[y] < 250) { // speed up by only checking non-white
										// pixels.
					double weightSum = 0;
					double weight = 0;
					double graySum = 0;
					for (int dx = Math.max(0, x - distance); dx < Math.min(
							raster.getWidth(), x + distance); dx++) {
						raster.getPixels(dx, 0, 1, raster.getHeight(), pColD);
						for (int dy = Math.max(0, y - distance); dy < Math.min(
								y + distance, raster.getHeight()); dy++) {
							if (dx != x && dy != y) {
								double d2 = (pCol[y] - pColD[dy])
										* (pCol[y] - pColD[dy]);
								// double d2 = (x - dx) * (x - dx) + (y - dy) *
								// (y - dy);
								weight = Math.exp(-1d / (h2)
										* Math.max(0d, d2 - sigma2));
								weightSum += weight;
	...
	public static BufferedImage removeScratches(BufferedImage img) {
		...
		ConvolveOp op = new ConvolveOp(new Kernel(5, 5,
		...
		for (int x = 0; x < raster.getWidth(); x++) {
			...
			for (int y = 0; y < raster.getHeight(); y++) {
				...
				if (p[0] < thresholdGray) { // thresholding
					...
					checkNeighbour(raster, x, y, thresholdGray, thresholdCount,
							count);
	...
	private static void checkNeighbour(WritableRaster raster, int x, int y,
			int thresholdGray, int thresholdCount, int[] count) {
		if (count[0] > thresholdCount)
			return;
		int[] pixel = new int[1];
		...
		if (pixel[0] < thresholdGray) {
			...
			if (count[0] < thresholdCount) {
				// checkNeighbour(raster, x, y, thresholdGray, thresholdCount,
				// count);
				checkNeighbour(raster, x, y + 1, thresholdGray, thresholdCount,
						count);
				checkNeighbour(raster, x + 1, y, thresholdGray, thresholdCount,
						count);
				checkNeighbour(raster, x + 1, y + 1, thresholdGray,
